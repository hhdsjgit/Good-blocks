...
import net.hhdsj.goodblock.init.GoodblockModMenus;

import java.util.function.Supplier;
import java.util.Map;
import java.util.HashMap;

public class CatalyzerguiMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public CatalyzerguiMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(GoodblockModMenus.CATALYZERGUI.get(), id);
		...
		if (pos != null) {
			if (extraData.readableBytes() == 1) { // bound to item
				...
				itemstack.getCapability(ForgeCapabilities.ITEM_HANDLER, null).ifPresent(capability -> {
					this.internal = (IItemHandler) capability;
					this.bound = true;
				});
		...
		this.customSlots.put(1, this.addSlot(new SlotItemHandler(internal, 1, 12, 60) {
		}));
		this.customSlots.put(2, this.addSlot(new SlotItemHandler(internal, 2, 159, 33) {
			@Override
